...
func (m *MachinePoolScope) HasReplicasExternallyManaged(ctx context.Context) bool {
	machinePoolHasOldAutoscalerAnnotation := false
	if value, ok := m.MachinePool.Annotations[azure.ReplicasManagedByAutoscalerAnnotation]; ok && value == "true" {
		machinePoolHasOldAutoscalerAnnotation = true
	}

	azureMachinePoolHasOldAutoscalerAnnotation := false
	if value, ok := m.AzureMachinePool.Annotations[azure.ReplicasManagedByAutoscalerAnnotation]; ok && value == "true" {
		azureMachinePoolHasOldAutoscalerAnnotation = true
	}

	return annotations.ReplicasManagedByExternalAutoscaler(m.MachinePool) || machinePoolHasOldAutoscalerAnnotation || azureMachinePoolHasOldAutoscalerAnnotation
}
